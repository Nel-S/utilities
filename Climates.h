...
void U_manualBNinit(BiomeNoise *bn, const bool largeBiomesFlag);
...
void U_initClimate(const int climate, PerlinNoise *oct, const uint64_t seed, const bool largeBiomesFlag);

// Initializes all climates using `U_initPerlin()`s.
void U_initAllClimates(PerlinNoise *oct, const uint64_t seed, const bool largeBiomesFlag);
...
double U_sampleClimate(const int climate, PerlinNoise *oct, double *px, double *pz, const bool largeBiomesFlag);
...
// If a non-`SHIFT` climate is sampled directly without sampling `NP_SHIFT` first, `*px` must be set to `floor(*px/4.)` and `*pz` to `floor(*pz/4.)` beforehand.
// Depth sampling is not supported due to that using splines in addition to Perlin octaves.
int U_sampleClimateBounded(const int climate, PerlinNoise *oct, double *px, double *pz, const double *lowerBounds, const double *upperBounds, const bool largeBiomesFlag, double *climateSample);
